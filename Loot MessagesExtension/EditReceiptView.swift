...
struct EditReceiptView: View {
    ...
    init(uiModel: LootUIModel, onSave: @escaping (ReceiptDisplay) -> Void, onCancel: @escaping () -> Void) {
        ...
        if receipt.taxCents > 0 {
            fees.append(EditableFee(id: UUID(), label: "Tax", amount: formatCents(receipt.taxCents)))
        }
        if receipt.feesCents > 0 {
            fees.append(EditableFee(id: UUID(), label: "Fees", amount: formatCents(receipt.feesCents)))
        }
        if receipt.tipCents > 0 {
            fees.append(EditableFee(id: UUID(), label: "Tip", amount: formatCents(receipt.tipCents)))
        }
        ...
        if receipt.discountCents > 0 {
            discountsList.append(EditableDiscount(id: UUID(), label: "Discount", amount: formatCents(receipt.discountCents)))
        }
        ...
        _totalString = State(initialValue: formatCents(receipt.totalCents))
    }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        VStack(spacing: 0) {
                            ForEach(taxesAndFees.indices, id: \.self) { idx in
                                ...
                                let isLast = idx == taxesAndFees.count - 1
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        VStack(spacing: 0) {
                            ForEach(discounts.indices, id: \.self) { idx in
                                ...
                                let isLast = idx == discounts.count - 1
